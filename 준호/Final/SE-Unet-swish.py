...
def conv2d_bn(x, filters, kernel_size, padding='same', strides=1, activation='Swish'):
...
def SE_block(input_tensor, reduction_ratio=16):
    ...
    # Excitation
    x = Dense(ch_reduced, kernel_initializer='he_normal', activation='Swish', use_bias=False)(x)
...
def SE_residual_block(input_tensor, filter_sizes, strides=1, reduction_ratio=16):
    ...
    shortcut = Activation(activation='Swish')(shortcut)
    return shortcut
...
class CallBacks(Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        # print()
        spath = "./models/SE-Res-Unet-epoch-{}-val-DSC-{}-Swish.hdf5".format(ep, vl_dsc)
        print(spath)
...
import pickle
with open('./trainres_dict_Swish.pkl', 'wb') as pkl:
    pickle.dump(history, pkl)