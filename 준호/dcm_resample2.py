...
def resample(img, org_spacing, new_spacing=[1, 1, 1]):
    resize_factors = []
...
for r_dpath in [os.path.join('E:/Dataset/Disc/Dataset', d) for d in os.listdir('E:/Dataset/Disc/Dataset')]:
    ...
    try:
        ...
        dataset_space = [float(dcm_spacing[0]), float(dcm_spacing[1]), float(dcm_thickness)]
        ...
        new_d_path = r_dpath.replace('Dataset/Disc', 'Dataset/Disc_2D_resampled')
        new_l_path = r_lpath.replace('Dataset/Disc', 'Dataset/Disc_2D_resampled')
