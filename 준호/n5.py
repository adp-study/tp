...
for train_index, validation_index in kf.split(train_input_split):
    ...
    fold_train_input = np.expand_dims(np.array([cv2.resize(img, (128, 128), interpolation=cv2.INTER_AREA) for img in fold_train_input_list]), axis=-1)
    fold_train_label = np.transpose(np.array([[cv2.threshold(cv2.resize(img, (128, 128), interpolation=cv2.INTER_AREA), 50, 255, cv2.THRESH_BINARY_INV)[1], cv2.threshold(cv2.resize(img, (128, 128), interpolation=cv2.INTER_AREA), 50, 255, cv2.THRESH_BINARY)[1]] for img in fold_train_label_list]), axes=[0, 2, 3, 1]) / 255
    ...
    fold_validation_label = np.transpose(np.array([[cv2.threshold(cv2.resize(img, (128, 128), interpolation=cv2.INTER_AREA), 50, 255, cv2.THRESH_BINARY_INV)[1], cv2.threshold(cv2.resize(img, (128, 128), interpolation=cv2.INTER_AREA), 50, 255, cv2.THRESH_BINARY)[1]] for img in fold_validation_label_list]), axes=[0, 2, 3, 1]) / 255
    print(fold_validation_input.shape, fold_validation_label.shape)

    # Train Each Fold
    model = Unet()
    model.summary()
    model.compile(optimizer=Adam(lr=1e-4), loss=dice_coef_loss, metrics=[iou_coef, dice_coef])
